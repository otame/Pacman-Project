...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        sat_assignment = []
        scope = [qi, qj]
        for value_1 in (scope[0]).domain():
            for value_2 in (scope[1]).domain():
                diff_values = value_1 - value_2
                abs_value_q = abs(value_1 - value_2)
                abs_value_ij = abs(i - j)
                diff_abs_values = abs_value_q - abs_value_ij
                if diff_values != 0 and diff_abs_values != 0:
                    sat_assignment.append([value_1, value_2])
                    sat_assignment.append([value_2, value_1])
        TableConstraint.__init__(self, name, scope, sat_assignment)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        temp = []
        while i < size:
            value = (self.scope()[i]).getValue()
            if value == self._required:
                temp.append(value)
            i += 1
        return self._lb <= len(temp) <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        # var should not in self.scope
        if var not in self.scope():
            return True

        def finalTestfn(assignment):
            size = len(assignment)
            i = 0
            temp = []
            while i < size:
                ...
                i += 1
            bound = len(temp)
            return self._lb <= bound <= self._ub

        def partialTestfn(assignment):
            size = len(assignment)
            i = 0
            temp = []
            while i < size:
                ...
                i += 1
            bound = len(temp)
            return bound <= self._ub

        assign_vars = self.scope()
        assign_vars.remove(var)
        return findvals(assign_vars, [(var, val)], finalTestfn, partialTestfn)