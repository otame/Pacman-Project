...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty():
        ...
        rel_solution = []
        for v in csp.variables():
            sol_tuple = (v, v.getValue())
            temp.append(sol_tuple)
        rel_solution.append(temp)
        return rel_solution
    all_solutions = []
    # next unAssignedVar
    ...
    for val in next_Var.curDomain():
        ...
        # set the constraint is statisfied
        constriant_satisfier = True
        for cnstraint in csp.constraintsOf(next_Var):
            if cnstraint.numUnassigned() == 1:
                if FCCheck(cnstraint, next_Var, val) == "DWO":
                    # set the constraint is not statisfied
                    constriant_satisfier = False
                    break
        # if the constraint is not statisfied
        if constriant_satisfier:
            # we have the new solutio by recurssively calling FC function
            new_solns = FC(unAssignedVars, csp, allSolutions, trace)
            if new_solns:
                all_solutions.extend(new_solns)
            empty = True
            if len(all_solutions) > 0: empty = False
            # if all_solutions are not empty and also allSolutions is False
            if not empty and not allSolutions:
